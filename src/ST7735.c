...
// Output: none
void ST7735_DrawCharS(int16_t x, int16_t 
y, char c, int16_t textColor, int16_t bgColor, uint8_t size){
...
void ST7735_OutUDec(uint32_t n){
  ...
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLUE, 1);
  }
}
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_YELLOW,ST7735_BLUE, 1);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLUE, 1);
...
void ST7735_DrawSquare(uint16_t x, uint16_t y, uint16_t length, uint32_t color){
	ST7735_DrawFastHLine(x,y,length,color);
	ST7735_DrawFastVLine(x,y,length,color);
	ST7735_DrawFastHLine(x,y+length,length,color);
	ST7735_DrawFastVLine(x+length,y,length,color);
}

uint32_t ST7735_DrawStringS(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t size){
  uint32_t count = 0;
	if(size == 1){
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLUE, size);
    pt++;
    x = x+1;
    if(x>20) return count;  // number of characters printed
    count++;
...
uint32_t ST7735_DrawStringUnfixed(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t size){
  uint32_t count = 0;
	uint16_t enhance = 0;
  if(y>150) return 0;
  while(*pt){
    ST7735_DrawCharS(enhance*6+x, y, *pt, textColor, ST7735_BLUE, size);
    pt++;
    enhance = enhance+1;
    if(enhance>20) return count;  // number of characters printed
    count++;
  }
  return count;  // number of characters printed
}
